  ...
  return (
    <section className={`companion-card mood-${stats.mood} ${compact ? "compact" : ""}`}>
      <div className={`dog-scene stage-${stats.stage}`} aria-label={`${name}、${stats.moodLabel}`}>
        ...
        <Image
          src={moodImage[stats.mood]}
          alt="緑のバンダナを着けたロングヘア・ダックスフント"
          width={360}
          height={300}
          className="dog-character-image"
        />
      ...
      <div className="companion-copy">
        <div className="companion-heading">
          <div><span className="status-pill"><i />{stats.moodLabel}</span><h2>{name}</h2></div>
        ...
        {!compact && (
          <div className="growth-area">
            <div className="growth-title"><span>{stats.nextStageName ? `次の成長：${stats.nextStageName}` : "最高のパートナーに成長しました"}</span><strong>{stats.nextStageName ? `あと${stats.chaptersToNextStage}章` : "100%"}</strong></div>
