...
type Props = { name: string; stats: CompanionStats; compact?: boolean };

const itemIcons: Record<string, string> = {
  "おそろいの名札": "🏷️", "ふかふかクッション": "🛏️", "みどりのバンダナ": "🧣",
  "小さな本棚": "📚", "旅の首輪": "🦴", "夕焼けの丘": "🌅",
};
  ...
  return (
    <section className={`companion-card mood-${stats.mood} ${compact ? "compact" : ""}`}>
      <div className={`dog-scene stage-${stats.stage}`} aria-label={`${name}、${stats.moodLabel}`}>
        ...
        <LayeredDachshund action={stats.mood} compact label={`緑のバンダナを着けた${name}`} />
      </div>
      <div className="companion-copy">
        <div className="companion-heading"><div><span className="status-pill"><i />{stats.moodLabel}</span><h2>{name}</h2></div><span className="stage-label">{stats.stageName}</span></div>
